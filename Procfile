...
bootstrap-spanner-pwd: cd cmd && bash -c 'for i in {1..300}; do (echo > /dev/tcp/localhost/${SPANNER_EMULATOR_PORT}) >/dev/null 2>&1 && sleep 1 && break || sleep 0.1; done;' && go run --tags=collect_resource_permissions,passwordApp cmd/bootstrap/main.go
